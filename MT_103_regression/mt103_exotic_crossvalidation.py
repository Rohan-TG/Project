...
JEFF = pd.read_csv('JEFF-3.3_MT_103_all_features.csv')
JEFF.index = range(len(JEFF))
JEFF_nuclides = range_setter(df=JEFF, la=0, ua=208)

JENDL = pd.read_csv('JENDL-5_MT_103_all_features.csv')
JENDL.index = range(len(JENDL))
JENDL_nuclides = range_setter(df=JENDL, la=0, ua=208)

CENDL = pd.read_csv('CENDL-3.2_MT_103_all_features.csv')
...
for q in tqdm.tqdm(range(num_runs)):
	...
	X_train, y_train = make_train(df=df, validation_nuclides=validation_nuclides, la=0, ua=208,
							minerg=minenergy, maxerg=maxenergy, mode='both')  # make training matrix

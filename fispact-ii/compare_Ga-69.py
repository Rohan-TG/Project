...
endfb80_values = np.array([1.65435e-06, 1.15693e-06, 1.04102e-06, 1.00076e-06,
    9.82130e-07, 9.67849e-07, 9.48835e-07, 9.26469e-07, 9.06201e-07,
    8.80697e-07, 8.51760e-07, 8.25650e-07, 7.90843e-07, 7.51364e-07,
    7.16049e-07, 6.70058e-07, 6.17314e-07, 5.71302e-07, 5.30501e-07,
    4.69590e-07, 3.99028e-07, 3.42831e-07])
endfb80_values = endfb80_values * 1e6
...
plt.plot(exp_minutes, endfb80_values, 'o-', label = 'ENDF/B-VIII.0')
plt.xscale('log')
plt.legend()
plt.grid()
# plt.yscale('log')
...
mlerrs = []
endfberrs = []
for tendl, ml, endfb, true in zip(TENDL_values, ML_values, endfb80_values, exp_decay_heat):
	...
	endfberr = ((endfb - true) / true ) * 100
	endfberrs.append(endfberr)


plt.figure()
plt.plot(exp_minutes, terrs, 'x-', label = 'TENDL error')
plt.plot(exp_minutes, mlerrs, 'x-', label = 'ML error')
plt.plot(exp_minutes, endfberrs, 'o-', label = 'ENDF/B-VIII error')
plt.grid()
plt.legend()
plt.show()
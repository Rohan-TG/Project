...
while upper_training_limit < 208:
	validation_nuclides = []

	nuclide_subset = range_setter(df=df, la=lower_training_limit, ua=upper_training_limit)

	while len(validation_nuclides) < validation_set_size:  # up to 25 nuclides
		choice = random.choice(nuclide_subset)  # randomly select nuclide from list of all nuclides in ENDF/B-VIII
		if choice not in validation_nuclides:
			validation_nuclides.append(choice)
	print("Test nuclide selection complete")

	X_train, y_train = make_train(df=df, validation_nuclides=validation_nuclides, exclusions= [],la=lower_training_limit, ua=upper_training_limit,) # create training matrix
	...
	for i, (pred_xs, true_xs, erg) in enumerate(zip(P_plotmatrix, XS_plotmatrix, E_plotmatrix)):
		...
		lower_training_limit += 10
		upper_training_limit += 10

		print(lower_training_limit, upper_training_limit)
		print()
		time.sleep(1)

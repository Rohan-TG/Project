...
from matrix_functions import dsigma_dE, make_train, make_test,\
	range_setter, General_plotter, r2_standardiser, rmse_standard
import scipy.stats
...
for i in tqdm.tqdm(range(n_evaluations)):
	...
	as1,as2, endfbr2 = r2_standardiser(predicted_xs=predictions, library_xs=XS_plotmatrix[0])
	...
	if target_nuclide in CENDL_nuclides:
		...
		cendl_rmses.append(rmse_standard(libxs=cendl_xs, pxs=pred_cendl))
	...
	if target_nuclide in JENDL_nuclides:
		...
		jendl_rmses.append(rmse_standard(libxs=jendl_xs, pxs=pred_jendl))
	...
	if target_nuclide in JEFF_nuclides:
		...
		jeff_rmses.append(rmse_standard(libxs=jeff_xs, pxs=pred_jeff))
	...
	if target_nuclide in TENDL_nuclides:
		...
		tendl_rmses.append(rmse_standard(libxs=tendl_xs, pxs=pred_tendl))
...
print(f"Predictions - ENDF/B-VIII R2: {pred_endfb_r2:0.5f} RMSE: ")
...
print(f"TENDL R2: {np.mean(tendl_r2s):0.3f} +- {np.std(tendl_r2s):0.3f}")
if target_nuclide in JEFF_nuclides:
	plt.plot(JEFF_energy, JEFF_XS, '--', label='JEFF3.3', color='mediumvioletred')
	print(f"JEFF R2: {np.mean(jeff_r2s):0.3f} +- {np.std(jeff_r2s):0.3f}, RMSE {np.mean(jeff_rmses):0.3f} +- {np.std(jeff_rmses):0.3f}")
if target_nuclide in JENDL_nuclides:
	plt.plot(JENDL5_energy, JENDL5_XS, label='JENDL5', color='green')
	print(f"JENDL R2: {np.mean(jendl_r2s):0.3f} +- {np.std(jendl_r2s):0.3f}")
if target_nuclide in CENDL_nuclides:
	plt.plot(CENDL32_energy, CENDL33_XS, '--', label = 'CENDL3.2', color='gold')
	print(f"CENDL R2: {np.mean(cendl_r2s):0.3f} +- {np.std(cendl_r2s):0.3f}")

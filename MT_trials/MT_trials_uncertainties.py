...
from matrix_functions import dsigma_dE, mtmake_train, mtmake_test,\
...
JENDL5 = pd.read_csv('JENDL5_with_MT16_and_ENDFB_MT103_107.csv')
JENDL5.index = range(len(JENDL5))
JENDL_nuclides = range_setter(df=JENDL5, la=0, ua=210)


CENDL32 = pd.read_csv('CENDL32_MT16_with_ENDFB_MT103_107.csv')
...
for i in tqdm.tqdm(range(n_evaluations)):
	...
	X_train, y_train = mtmake_train(df=df, validation_nuclides=validation_nuclides,
								  exclusions=[],
								  la=0, ua=100,) # make training matrix

	X_test, y_test = mtmake_test(validation_nuclides, df=df_test,)
	...
	if target_nuclide in CENDL_nuclides:
		...
		cendl_test, cendl_xs = mtmake_test(nuclides=[target_nuclide], df=CENDL32)
	...
	if target_nuclide in JENDL_nuclides:
		...
		jendl_test, jendl_xs = mtmake_test(nuclides=[target_nuclide], df=JENDL5)
	...
	if target_nuclide in JEFF_nuclides:
		jeff_test, jeff_xs = mtmake_test(nuclides=[target_nuclide], df=JEFF33)
	...
	if target_nuclide in TENDL_nuclides:
		...
		tendl_test, tendl_xs = mtmake_test(nuclides=[target_nuclide], df=TENDL)

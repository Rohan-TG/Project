...
def make_train(df, validation_nuclides, exclusions = [], la=0, ua=260, use_tqdm=False):
	...
	if use_tqdm:
		iterator = tqdm.tqdm(enumerate(Z), total=len(Z))
	else:
		iterator = enumerate(Z)

	for idx, unused in iterator:  # MT = 16 is (n,2n) (already extracted)
...
def make_test(nuclides, df):
	...
	for nuc_test_z, nuc_test_a in zip(ztest, atest):
		for j, (zval, aval) in enumerate(zip(Z, A)):
			if zval == nuc_test_z and aval == nuc_test_a and Energy[j] <= 20 and not math.isnan(XS[j]):
				Z_test.append(Z[j])
				A_test.append(A[j])
